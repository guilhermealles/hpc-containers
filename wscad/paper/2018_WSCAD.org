...
\begin{figure}[!htb]
\centering
\includegraphics[width=\textwidth]{./img/pingpong-latency.pdf}
...
#+begin_src R :results output graphics :file img/pingpong-latency.pdf :exports both :width 4 :height 3.8 :session
#Create a custom color scale
library(RColorBrewer)
myColors <- brewer.pal(5,"Set1")
...
#+begin_src R :results output graphics :file img/pingpong-overhead.pdf :exports both :width 4 :height 3.8 :session
df.pingpong.overhead %>%
    mutate(environment = factor(gsub(".Overhead", "", environment),
                                levels=c("Singularity", "Docker"))) -> df.sel;
df.sel %>%
    ...
    geom_line(alpha = 0.3) +
    xlab('Message size (bytes)') + 
    ylab('Latency Overhead against Native (%)') +
    theme_bw(base_size = 13) +
    theme (plot.margin = unit(c(0,0,0,0), "cm"),
           legend.spacing = unit(x = c(0, 0, 0, 0), units = 'mm'),
           legend.position = "top",
