...
\centering
\subfloat[Execution Time
\label{fig2:left}]{\begin{minipage}[c]{.47\linewidth}
\includegraphics[width=.90\linewidth]{./img/computation-ligurian.pdf} \hfill
\end{minipage}}\hfill%
%
\subfloat[Overhead against Native
\label{fig2:right}]{\begin{minipage}[c]{.47\linewidth}
\includegraphics[width=.90\linewidth]{./img/computation-overhead-large.pdf} \\
\end{minipage}}\hfill%
\caption{(a) Execution time as a function of the number of MPI ranks
...
\ref{fig3:left} depicts the average latency (on Y axis) between two
nodes for the three environments (color) as a function of the message
size (on X). From these results, we can see that the network
performance on Docker containers is considerably lower when compared
to both the native and singularity test cases. This evidence confirms
...
environment against the native OS. We can see that the overhead
imposed by Singularity in the communication latency remains stable no
matter which message size, which is something desirable.  In some
...
that low scalability of the approach, especially for those
applications with larger message sizes. For instance, in the case of
Ondes3D that we previously studied, processes exchange multiple small
message sizes (depending on the domain decomposition). Even if most
...
\label{fig3:left}]{\begin{minipage}[c]{.49\linewidth}
\includegraphics[width=.97\linewidth]{./img/pingpong-latency.pdf} \hfill
\end{minipage}}\hfill%
%
\subfloat[Latency Overhead
\label{fig3:right}]{\begin{minipage}[c]{.49\linewidth}
\includegraphics[width=.97\linewidth]{./img/pingpong-overhead.pdf} \\
\end{minipage}}\hfill%
...
*** Performance Gains with an Optimized Container
