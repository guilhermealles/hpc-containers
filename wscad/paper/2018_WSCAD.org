...
\begin{figure}[!htb]
\centering
\subfloat[Smaller experiment (up to 4 nodes, 16 cores)
\label{fig1:left}]{\begin{minipage}[b]{.47\linewidth}
\includegraphics[width=\linewidth]{./img/computation.pdf} \hfill
\end{minipage}}\hfill%
%
\subfloat[Larger experiment (up to 64 nodes, 256 cores)
\label{fig1:right}]{\begin{minipage}[b]{.47\linewidth}
\includegraphics[width=\linewidth]{./img/computation-ligurian.pdf} \\
\end{minipage}}\hfill%
...
**** Plots (Lucas)
***** Computation Analysis (pure makespan)
****** Read and merge data
...
df.makespan <- do.call("bind_rows", lapply(files, function(file) {
    ...
    mutate(environment = factor(environment,
                                levels=c("native", "docker", "singularity"),
                                labels=c("Native", "Docker", "Singularity")))
#+end_src
...
df.makespan %>%
    ...
    filter(Input %in% c("Class B", "Default")) %>%
    ...
    xlab('Number of MPI processes (count)') + 
    ylab('Execution time (s)') +
    theme(legend.position = 'top', legend.spacing = unit(x = c(0, 0, 0, 0), units = 'mm')) +
...
****** (right) Plot computation of the Ondes3D/Ligurian
****** Ondes3D Ligurian plot
#+begin_src R
library(tidyverse);
default_theme <- function() {
