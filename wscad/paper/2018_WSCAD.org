...
to drive experiments and collect measurements. The first design
targets a smaller scale test using up to four nodes, with 1, 4, 8, and
16 processes; the second design uses 64 nodes, with 64, 128, 192, and
256 processes. The first smaller batch uses NAS-EP executed with the
Class B workload and Ondes3D with the default test case. The second
larger batch of experiments are conducted with Ondes3D using the
Ligurian workload. The reported makespan measurements are averages from 10 to
30 replications of each experiment parameter configuration; error bars
are calculated considering a confidence level of 99% assuming a
