...
Two different test suites were run. One containing all of the benchmarks cited above and other considering a real world, large Ondes3D simulation. All of the tests were run in a full factorial design. The first suite had the following factors:
 - Execution environment: Native, Docker and Singularity
 - Parallel compute units: 1, 4, 8, 16

The second suite had the following factors:
