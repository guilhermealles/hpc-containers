...
against the native runs, also for both applications. For the
*NAS-EP/ClassB* case, although indicating an advantage in the native
execution, the plot shows that the virtualized approaches perform very
close to each other and to the native baseline. The overhead of any
container is less than \approx10% for up to 16 cores, but with an increasing
trend. This indicates that regarding CPU calculation, none of the
...
MPI ranks) increases. Since these runs lasted for not so long -- about
15s for NAS-EP/ClassB, and 10s for Ondes3D/Default -- such overhead
may be absorbed with longer CPU-bound executions that make a limited
