...
is probably due to the diminishing returns on speedup as the number of
cores increase, which can mask the container initialization time with
other overheads that are the constant in both environments, such as
...
surprising but are still unconventional. The Linux container support
is designed to offer the virtualized environment the smallest possible
footprint from the native OS. Such a feature, combined with an
HPC-tailored container, can bring performance advantages as well as a
