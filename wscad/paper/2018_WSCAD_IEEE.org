...
#+LATEX_CLASS_OPTIONS: [conference,a4paper,10pt,final]
...
\def\COMPANION{{\small\url{https://github.com/guilhermealles/hpc-containers/}}}
#+END_EXPORT
...
\IEEEauthorblockA{Instituto de Informática -- Universidade Federal do Rio Grande do Sul (UFRGS)\\
...
measurements, is publicly available at \linebreak \COMPANION.
...
\centering
\subfloat[Execution Time
\label{fig2:left}]{\begin{minipage}{.8\linewidth}
\includegraphics[width=.90\linewidth,valign=t]{./img/computation-ligurian.pdf} \hfill
\end{minipage}}\\
\subfloat[Overhead against Native
\label{fig2:right}]{\begin{minipage}{.8\linewidth}
...
#+begin_src R :results output graphics :file img/v2-network-latency-raw.pdf :exports both :width 6.4 :height 3 :session
#Create a custom color scale
library(RColorBrewer)
colors <- brewer.pal(9,"Set1")
myColors <- c(colors[1], colors[3], colors[2])
...
#+RESULTS:
[[file:img/v2-network-latency-raw.pdf]]
...
#+begin_src R :results output graphics :file img/v2-alpine-nas-ondes3d-raw.pdf :width 4 :height 3.8 :session
df.alpine %>%
    mutate(Native = factor(Native, levels=c(TRUE, FALSE), labels=c("Native", "Container"))) %>%
    mutate(Application = paste(Application, Input, sep="/")) %>%
    filter(Input %in% c("Class B", "Default")) -> df.sel;
...
#+RESULTS:
[[file:img/v2-alpine-nas-ondes3d-raw.pdf]]
...
3            8  NAS-EP/Class B      Docker.Gain  -6.32
4           16  NAS-EP/Class B      Docker.Gain  -4.62
5            1 Ondes3D/Default      Docker.Gain  -3.58
6            4 Ondes3D/Default      Docker.Gain  11.42
7            8 Ondes3D/Default      Docker.Gain  78.97
8           16 Ondes3D/Default      Docker.Gain 137.08
9            1  NAS-EP/Class B Singularity.Gain  -9.48
10           4  NAS-EP/Class B Singularity.Gain  -9.56
11           8  NAS-EP/Class B Singularity.Gain  -7.12
12          16  NAS-EP/Class B Singularity.Gain  -4.65
13           1 Ondes3D/Default Singularity.Gain  -3.55
14           4 Ondes3D/Default Singularity.Gain  -4.12
15           8 Ondes3D/Default Singularity.Gain  -3.68
16          16 Ondes3D/Default Singularity.Gain  -3.62
#+end_example
...
#+begin_src R :results output graphics :file img/v2-alpine-nas-ondes3d-overhead.pdf :width 3 :height 3.8 :session
df.alpine.overhead %>%
    filter(parallelism <= 16) %>%
    mutate(environment = factor(gsub(".Gain", "", environment),
                                levels=c("Singularity", "Docker"))) -> df.sel;
...
#+RESULTS:
[[file:img/v2-alpine-nas-ondes3d-overhead.pdf]]
