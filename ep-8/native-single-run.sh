...
if [ $CONTEXT = "mpi" ]; then
    EXEC_TIME=$($SOFTWARE_UTILS_DIR/ms-time.sh mpirun --hostfile $HOSTFILE -np $PARALLELISM $LOCAL_NAS_BUILD/NPB3.3.1/NPB3.3-MPI/bin/ep.B.16)
elif [ $CONTEXT = "mpi-high-comm" ]; then
    EXEC_TIME=$($SOFTWARE_UTILS_DIR/ms-time.sh mpirun --hostfile $HOSTFILE_FORCE_COMM -np $PARALLELISM $LOCAL_NAS_BUILD/NPB3.3.1/NPB3.3-MPI/bin/ep.B.16)
else # OpenMP
