...
do
    ...
    if [ $environment = "singularity" ]; then
        if [ $context = "mpi" ]; then
            EXEC_COMMAND="time mpirun --hostfile $HOSTFILE -np $parallelism ./images/mpi-c-$parallelism.img"
        elif [ $context = "mpi-high-comm" ]; then
            EXEC_COMMAND="time mpirun --hostfile $HOSTFILE_FORCE_COMM -np $parallelism ./images/mpi-c-$parallelism.img"
        else #OpenMP
            EXEC_COMMAND="time ./images/omp-c.img"
        fi
    elif [ $environment = "docker" ]; then
        if [ $context = "mpi" ]; then
            ./setup/assemble-swarm.sh create $HOSTFILE
            ./setup/ep-start-docker-cluster.sh up $parallelism
            EXEC_COMMAND="time ./swarm.sh exec mpirun -np $parallelism NPB3.3.1/NPB3.3-MPI/bin/ep.B.$parallelism"
        elif [ $context = "mpi-high-comm" ]; then
            ...
            EXEC_COMMAND="time ./swarm.sh exec NPB3.3.1/NPB3.3-OMP/bin/ep.B.x"
        fi
        cd $DOCKER_CLUSTER_DIR
    else # native
        if [ $context = "mpi" ]; then
            EXEC_COMMAND="time mpirun --hostfile $HOSTFILE -np $parallelism $LOCAL_NAS_BUILD/NPB3.3.1/NPB3.3-MPI/bin/ep.B.$parallelism"
        elif [ $context = "mpi-high-comm" ]; then
            EXEC_COMMAND="time mpirun --hostfile $HOSTFILE_FORCE_COMM -np $parallelism $LOCAL_NAS_BUILD/NPB3.3.1/NPB3.3-MPI/bin/ep.B.$parallelism"
        else # OpenMP
            EXEC_COMMAND="time '$LOCAL_NAS_BUILD/NPB3.3.1/NPB3.3-OMP/bin/ep.B.x'"
        fi
    fi
